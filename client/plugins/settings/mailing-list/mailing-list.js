Template.adminMailingList.rendered = function() {
  	$('#employees').DualListBox({},[]);
};

Template.adminMailingList.helpers({
	mail_lists: function() {
		return Mailing_Lists.find();
	},

	mail_list: function(){
		return Mailing_Lists.findOne(Session.get('selectedMailListId'));
	}
});


Template.adminMailingList.events({
	'submit #form-mail-list': function(e) {
		...
		if (mode == "create"){
			Meteor.call('addMailingList', params, function(error, result){
				if(error){
			...
			Meteor.call('updateMailingList', mailListId, params, function(error, result){
				if(error){
					console.log("error", error);
	...
	'click .edit-mail-list': function(e) {
		...
		var mail_list = Mailing_Lists.findOne(mailListId);
	...
	'click .delete-mail-list': function(e) {
		...
		notice.get().on('pnotify.confirm', function() {
			Meteor.call('deleteMailingList', mailListId, function(error, result){
				if(error) {
					console.log("error", error);
