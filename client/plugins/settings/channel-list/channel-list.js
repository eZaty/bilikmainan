Template.adminChannelList.rendered = function() {
  //Meteor.typeahead.inject();
  $('#channel-editors').select2();
};

Template.adminChannelList.helpers({
	channels: function() {
		return Channels.find();
	},

	channel: function(){
		return Channels.findOne(Session.get('selectedChannelId'));
	},

	editors: function() {
    	return Meteor.users.find({roles: 'channel-editor'}).fetch().map(function(object){ return {id: object._id, name: object.profile.name}; });
  	},

 	channel_cover_thumb: function(id){
 		return channel_cover_photo(id,'thumbs');
 	}
});


Template.adminChannelList.events({
	'submit #form-channel': function(e) {
		...
		if (mode == "create"){
			Meteor.call('addChannel', params, function(error, result){
				...
				if(result){
					...
					if ($('#channel-cover-photo').val()!=""){
				      ...
				      newCover.fileType = 'channel_cover_photo';
				      newCover.status = 'uploading';

				      Channel_Images.insert(newCover, function(err, fileObj){
				          if(err){
				    ...
				    if ($('#channel-footer-photo').val()!=""){
				      ...
				      newFooter.fileType = 'channel_footer_photo';
				      newFooter.status = 'uploading';

				      Channel_Images.insert(newFooter, function(err, fileObj){
				          if(err){
			...
			Meteor.call('updateChannel', channelId, params, function(error, result){
				if(error){
					console.log("error", error);
					var msg = 'Failed to update channel.';
					ClientHelper.notify('danger', msg, true);
				}
				if(result){
					var msg = 'Channel successfully updated.';
					...
					if ($('#channel-cover-photo').val()!=""){
				      ...
				      Meteor.call('removeChannelCoverImage', channelId, function(err, res){
				    ...
				    if ($('#channel-footer-photo').val()!=""){
				      ...
				      Meteor.call('removeChannelFooterImage', channelId, function(err, res){
	...
	'click .edit-channel': function(e) {
		...
		$('#preview-cover-photo').prop('src', channel_cover_photo(channelId,'full'));
		$('#preview-footer-photo').prop('src', channel_footer_photo(channelId,'full'));
