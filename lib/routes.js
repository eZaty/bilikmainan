...
Router.route('/__admin/announcement/mailing-list', {
    name: 'adminAnnouncementMailingList',
    layoutTemplate: 'adminLayout',
    waitOn: function () {
        if(Meteor.isClient){
            return [
    ...
    onBeforeAction: function() {
        if (!Meteor.userId()) {
            Router.go('/login');
        } else if(!Roles.userIsInRole(Meteor.userId(), ['admin','announcement-editor'])) {
            Router.go('/');
        } else {
            this.next();
        }
...
Router.route('/__admin/announcement/channels', {
    name: 'adminAnnouncementChannelList',
    layoutTemplate: 'adminLayout',
    waitOn: function () {
        return [
            Meteor.subscribe('roles'),
...
Router.route('/__admin/announcement/mine', {
    name: 'adminMyAnnouncement',
    layoutTemplate: 'adminLayout',
    waitOn: function () {
        return [
            Meteor.subscribe('roles'),
...
Router.route('/webetv2', {
    ...
    waitOn: function () {
        if(Meteor.isClient){
            return [
                IRLibLoader.load('/scripts/jquery.vticker.min.js'),
                // IRLibLoader.load('/scripts/css-clock.js'),
                Meteor.subscribe('timelines'),
                // Meteor.subscribe('blog_images'),
                // Meteor.subscribe('announcement_images'),
                // Meteor.subscribe('announcement_videos'),
